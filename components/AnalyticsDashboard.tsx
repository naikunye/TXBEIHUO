...
import { TrendingUp, Package, Zap, LineChart, BarChart2, AlertTriangle, PieChart, BrainCircuit, DollarSign } from 'lucide-react';
...
export const AnalyticsDashboard: React.FC<AnalyticsDashboardProps> = ({ records }) => {
  ...
  const structureStats = useMemo(() => {
      ...
      records.forEach(r => {
          ...
          const m = calculateMetrics(r);
          // Capital = Product Cost + Logistics Cost (Approx in CNY)
  ...
  // --- CHART 1: Structure Chart (Donut) ---
  const renderStructureChart = () => {
      const data = [
          { id: 'New', label: '新品推广', value: structureMode === 'count' ? structureStats.New.count : structureStats.New.capital, color: '#3b82f6' },
          { id: 'Growth', label: '高速增长', value: structureMode === 'count' ? structureStats.Growth.count : structureStats.Growth.capital, color: '#10b981' },
          { id: 'Stable', label: '稳定热卖', value: structureMode === 'count' ? structureStats.Stable.count : structureStats.Stable.capital, color: '#6366f1' },
          { id: 'Clearance', label: '清仓处理', value: structureMode === 'count' ? structureStats.Clearance.count : structureStats.Clearance.capital, color: '#ef4444' },
      ].filter(d => d.value > 0);
                      ...
                      {data.map((d, i) => {
                          ...
                          return (
                              <path 
                                ...
                                stroke="#0f172a" 
                                strokeWidth="0.02" 
                                className="hover:opacity-90 transition-transform duration-300 hover:scale-105 origin-center cursor-pointer"
                              />
                          );
                      })}
                  </svg>
  ...
  // --- CHART 3: Unit Economics Breakdown (Restored) ---
          ...
          <div className="space-y-6">
              ...
              <div className="flex flex-wrap gap-4 justify-center mt-4 pt-4 border-t border-gray-100 dark:border-white/5">
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                      <div className="w-2.5 h-2.5 rounded-full bg-blue-500"></div> 货值
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                      <div className="w-2.5 h-2.5 rounded-full bg-orange-500"></div> 头程
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                      <div className="w-2.5 h-2.5 rounded-full bg-purple-500"></div> 尾程
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                      <div className="w-2.5 h-2.5 rounded-full bg-pink-500"></div> 佣金
                  </div>
                  <div className="flex items-center gap-1.5 text-xs text-slate-500 dark:text-slate-400">
                      <div className="w-2.5 h-2.5 rounded-full bg-slate-500"></div> 广告
                  </div>
              </div>
          </div>
      );
            ...
            {chartData.map((item, idx) => (
              <div key={idx} className="relative group">
                ...
                <div className="w-full bg-slate-800 rounded-full h-2 overflow-hidden flex shadow-inner">
                   <div className="bg-slate-600 h-full opacity-50" style={{ width: `${(item.cost / (item.cost + Math.abs(item.profit))) * 100}%` }}></div>
                   <div className={`h-full transition-all duration-500 relative ${item.profit > 0 ? 'bg-emerald-500 shadow-[0_0_8px_#10b981]' : 'bg-red-500 shadow-[0_0_8px_#ef4444]'}`} style={{ width: `${(Math.abs(item.profit) / (item.cost + Math.abs(item.profit))) * 100}%` }}>
