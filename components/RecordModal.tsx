...
export const RecordModal: React.FC<RecordModalProps> = ({ isOpen, onClose, onSave, initialData, stores, defaultStoreId }) => {
  const [activeTab, setActiveTab] = useState<'details' | 'history'>('details'); // New Tab State
  ...
  const handleSubmit = (e: React.FormEvent) => {
    ...
    if (initialData) {
        ...
        if (initialData.quantity !== formData.quantity) changes.push(`库存: ${initialData.quantity} -> ${formData.quantity}`);
        if (initialData.unitPriceCNY !== formData.unitPriceCNY) changes.push(`采购价: ¥${initialData.unitPriceCNY} -> ¥${formData.unitPriceCNY}`);
        if (initialData.salesPriceUSD !== formData.salesPriceUSD) changes.push(`售价: $${initialData.salesPriceUSD} -> $${formData.salesPriceUSD}`);
    ...
    <div className="fixed inset-0 z-50 flex items-center justify-center bg-black bg-opacity-50 backdrop-blur-sm p-4 animate-fade-in">
      <div className="bg-white rounded-2xl shadow-2xl w-full max-w-4xl max-h-[90vh] overflow-hidden flex flex-col">
          ...
          <div className="flex items-center gap-4">
              ...
              <div className="flex bg-gray-200 p-1 rounded-lg">
                  <button 
                    ...
                    className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all flex items-center gap-1 ${activeTab === 'details' ? 'bg-white shadow text-gray-800' : 'text-gray-500 hover:text-gray-700'}`}
                  >
                      <FileText size={14}/> 详情
                  </button>
                  <button 
                    onClick={() => setActiveTab('history')}
                    className={`px-3 py-1.5 text-xs font-bold rounded-md transition-all flex items-center gap-1 ${activeTab === 'history' ? 'bg-white shadow text-blue-600' : 'text-gray-500 hover:text-gray-700'}`}
                  >
        ...
        <div className="flex-1 overflow-y-auto">
            {activeTab === 'details' ? (
                <form id="recordForm" onSubmit={handleSubmit} className="p-6 grid grid-cols-1 md:grid-cols-2 gap-8">
                  ...
                  <div className="space-y-4 p-5 bg-gray-50 rounded-xl border border-gray-100">
                     ...
                     <div className="grid grid-cols-2 gap-4">
                         <div>
                            ...
                            <input required type="number" step="1" name="materialCostCNY" value={formData.materialCostCNY} onChange={handleChange} className={inputClass} />
                         </div>
                         <div>
                            <label className={labelClass}>报关费 (¥)</label>
                            <input required type="number" step="1" name="customsFeeCNY" value={formData.customsFeeCNY} onChange={handleChange} className={inputClass} />
                         </div>
                         <div>
                            <label className={labelClass}>港口/操作费 (¥)</label>
                            <input required type="number" step="1" name="portFeeCNY" value={formData.portFeeCNY} onChange={handleChange} className={inputClass} />
                         </div>
                         <div className="md:col-span-1">
                ...
                <div className="p-6">
                    {(!formData.history || formData.history.length === 0) ? (
                        <div className="text-center py-20 text-gray-400">
                            ...
                            <p className="text-xs mt-1">所有修改操作将自动记录在此，确保数据安全。</p>
                        </div>
                    ) : (
                        <div className="relative border-l border-gray-200 ml-4 space-y-6">
                            {formData.history.map((log, index) => (
                                <div key={index} className="relative pl-6">
                                    <div className={`absolute -left-[9px] top-0 w-4 h-4 rounded-full border-2 border-white ${log.action === 'Create' ? 'bg-green-500' : 'bg-blue-500'}`}></div>
                                    <div className="flex items-center justify-between mb-1">
                                        <div className="flex items-center gap-2">
                                            <span className={`text-xs font-bold px-2 py-0.5 rounded text-white ${log.action === 'Create' ? 'bg-green-500' : 'bg-blue-500'}`}>{log.action}</span>
        ...
        {/* Footer Actions */}
        {activeTab === 'details' && (
            <div className="col-span-1 md:col-span-2 px-6 py-4 border-t border-gray-100 bg-white shrink-0">
                <button 
                    type="submit" 
                    form="recordForm" // Link external button to form
                    className="w-full bg-blue-600 text-white font-bold py-3.5 px-4 rounded-xl hover:bg-blue-700 transition duration-300 shadow-lg hover:shadow-xl transform hover:-translate-y-0.5 flex items-center justify-center gap-2"
                >
                  <Check size={18} />
                  {initialData ? '保存修改并记录日志' : '确认添加'}
                </button>
            </div>
        )}
      </div>
